...
function getNavMenuItems(menusData, initIndex, auth) {
  ...
  return menusData
    .filter((item) => {
      ...
      return item.name && !item.hideInMenu && roleAuth && item.show;
    })
    .map((item, index) => {
...
function getSubMenuOrItem(item, index, auth) {
  ...
  const navItem = (
    <NavItem key={item.path} icon={<div style={{ display: 'inline-block', marginRight: '6px' }}><img src={`../image/${item.icon}.png`} style={{ width: '28px', height: '28px', cursor: 'pointer' }} /></div>}>
...
const Navigation = (props, context) => {
  ...
  const { location, asideMenuConfig } = props;
